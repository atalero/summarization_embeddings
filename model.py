...
gloveFile = "./embeddings/glove.twitter.27B/glove.twitter.27B.50d.txt"
...
class SummarizationModel(object):
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      # Some initializers
      self.rand_unif_init = tf.initializers.random_uniform(-hps.rand_unif_init_mag, hps.rand_unif_init_mag, seed=123)
      self.trunc_norm_init = tf.initializers.truncated_normal(stddev=hps.trunc_norm_init_std)
      ...
      if FLAGS.glove:
        ...
        with tf.variable_scope('embedding'):
          embedding = tf.get_variable('embedding', dtype=tf.float32, initializer=glove_cons,trainable=False)
